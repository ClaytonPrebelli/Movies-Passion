<?php
function exibe_busca($nome,$genero){
  $nome_formatado = preg_replace('/[ -]+/' , '%20' , $nome);
$images_path = 'https://image.tmdb.org/t/p/w500';
$api_key = '4ec327e462149c3710d63be84b81cf4f';
if($nome !=""){
  $url = "https://api.themoviedb.org/3/search/movie?api_key=".$api_key."&language=pt-BR&sort_by=original_title.asc&query=".$nome_formatado."&page=1&with_genres=".$genero."&include_adult=false";
}else{
  $url="https://api.themoviedb.org/3/discover/movie?api_key=".$api_key."&language=pt-BR&sort_by=original_title.asc&sort_by=release_date.desc&include_adult=false&include_video=false&page=1&with_genres=".$genero."&with_watch_monetization_types=flatrate";
}
